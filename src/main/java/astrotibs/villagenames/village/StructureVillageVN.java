...
public class StructureVillageVN
{
    ...
    public static IBlockState getBiomeSpecificBlockState(IBlockState blockstate, MaterialType materialType, BiomeGenBase biome, boolean disallowModSubs)
    {
        ...
        case JUNGLE:
        	...
        	if (block == Blocks.stone_stairs)
													     	   {
        															   Block modstate = ModObjects.chooseModMossyCobblestoneStairsBlock();
																	   if (modstate!=null) {blockstate=modstate.getDefaultState();}
																	   break;
													     	   }
        	if (block == Blocks.stone_brick_stairs)
													     	   {
																	   Block modstate = ModObjects.choosModMossyStoneBrickStairsBlock();
																	   if (modstate!=null) {blockstate=modstate.getDefaultState();}
																	   break;
													     	   }
        ...
        case SAND:
        	...
        	if (block != null && block == ModObjects.chooseModBlueIceBlockState().getBlock()) {blockstate=Blocks.sandstone.getStateFromMeta(2); break;} // Chiseled sandstone
     ...
     */
    public static int getSignRotationMeta(int relativeOrientation, int horizIndex, boolean isHangingOnWall)
    {
    	if(isHangingOnWall)
    	{
    ...
    public static int setPathSpecificBlock(World world, MaterialType materialType, BiomeGenBase biome, boolean disallowModSubs, int posX, int posY, int posZ, boolean searchDownward)
    {
    	...
    	do
    	{
    				...
    				&& world.isAirBlock(new BlockPos(posX, surfaceY, posZ).up()))
    		{
				...
				for (int i=1; i<=4; i++)
				{
					if (surfaceY-i > 0 && world.getBlockState(pos.down(i)).getBlock() == ModObjects.chooseModPathState().getBlock() && world.getBlockState(pos.down(i)).getBlock() != Blocks.gravel)
					{
					...
					)
    		{
    			...
    			{
    				while(yDownScan > 0 && world.getBlockState(new BlockPos(posX, --yDownScan, posZ)).getBlock().getMaterial().isLiquid())
    				{
    ...
    public static class StartVN extends StructureVillagePieces.Start
    {
		...
		public void clearSpaceAbove(World world, StructureBoundingBox structureBB, int STRUCTURE_WIDTH, int STRUCTURE_DEPTH, int GROUND_LEVEL) {
		...
		public void establishFoundation(World world, StructureBoundingBox structureBB, String[] foundationPattern, int GROUND_LEVEL, MaterialType materialType, boolean disallowModSubs,
				BiomeGenBase biome, IBlockState biomeTopState, IBlockState biomeFillerState) {
	    		...
	    		else if (unitLetter.equals("P"))
	    		{
	    			...
	    			this.replaceAirAndLiquidDownwards(world, biomeFillerState, u, GROUND_LEVEL-1+(world.isAirBlock(new BlockPos(posX, posY, posZ))?0:-1), w, structureBB);
	    			StructureVillageVN.setPathSpecificBlock(world, materialType, biome, disallowModSubs, posX, posY, posZ, false);
	    		}
	    		else if (
	    				world.getBlockState(new BlockPos(posX, posY, posZ)).getBlock()==biomeFillerState.getBlock()
	    				|| world.getBlockState(new BlockPos(posX, posY, posZ)).getBlock()==biomeTopState.getBlock()
	    				)
	    		{
	...
	public static class VNComponent extends StructureVillagePieces.Village
	{
		...
		public void ascertainVillageStatsFromStartPiece(StartVN start) {
        	if (start!=null)
            {
    ...
    public static class DecorTorch extends StructureVillageVN.VNComponent
    {
...
//            super(start, componentType);
	 ...
	 * -X: returns the value X - used for things like upright barrels
	 */
	public static int chooseLadderMeta(int orientation, EnumFacing coordBaseMode) {return chooseFurnaceMeta(orientation, coordBaseMode);} // Same as Furnace in 1.7 and 1.8
