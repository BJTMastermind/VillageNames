...
public class StructureVillageVN
{
	...
	// Array of meta values for furnaces indexed by [orientation]
    ...
    public static EntityVillager makeVillagerWithProfession(World world, Random random, int profession, int career, int age)
    {
    	...
    	while(true)
    	{
    		...
    		if (profession==-1)
    		{
    			...
    			{
    				entityvillager.setProfession(random.nextInt(5)); //GeneralConfig.enableNitwit ? random.nextInt(6) : random.nextInt(5)); // TODO - Enable Nitwit setting in 1.11
    			}
    ...
    public static class StartVN extends StructureVillagePieces.Start
    {
        ...
        public StartVN(BiomeProvider biomeProvider, int componentType, Random random, int posX, int posZ, List components, float villageSize)
        {
			...
			try {
            	String mappeddisallowModSubs = (String) (mappedBiomes.get("DisallowModSubs")).get(mappedBiomes.get("BiomeNames").indexOf(biome.getBiomeName()));
		...
		public void establishFoundation(World world, StructureBoundingBox structureBB, String[] foundationPattern, int GROUND_LEVEL, MaterialType materialType, boolean disallowModSubs,
				Biome biome, IBlockState biomeTopState, IBlockState biomeFillerState) {
...
//            super(start, componentType);
        ...
        public boolean addComponentParts(World world, Random random, StructureBoundingBox structureBB)
        {
            ...
            for (int j=0; j<decorUVW.length; j++)
            {
            	...
            	case DESERT:
            		decorBlueprint = DesertStructures.getDesertDecorBlueprint(0, this.materialType, this.disallowModSubs, this.biome, this.getCoordBaseMode(), randomFromXYZ);
            		break;
            	case TAIGA:
