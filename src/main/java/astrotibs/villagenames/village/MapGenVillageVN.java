...
public class MapGenVillageVN extends MapGenVillage
{
    ...
    public MapGenVillageVN(Map par1Map)
    {
        ...
        while (iterator.hasNext())
        {
            ...
            {
                this.size = (float) MathHelper.parseDoubleWithDefaultAndMax((String)entry.getValue(), this.size, 0);
            }
    ...
    protected boolean canSpawnStructureAtCoords(int chunkXin, int chunkZin)
    {
        ...
        {
        	Biome biome = this.worldObj.getBiomeGenForCoords(new BlockPos(chunkX * 16 + 8, 0, chunkZ * 16 + 8)); // Biome doesn't care about Y
