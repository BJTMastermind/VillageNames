...
/*
 * Adapted from net.minecraftforge.common.ChestGenHooks in 1.8
 */
public class ChestGenHooks
{
    ...
    {
        if (item.theItemId!=null) {contents.add(item);} // Added null check just in case this becomes a problem in later versions
    }
    ...
    public List<WeightedRandomChestContent> getItems(Random random)
    {
        ...
        for (WeightedRandomChestContent wrcc : contents)
        {
            Item item = wrcc.theItemId.getItem();

            if (item != null)
            {
                WeightedRandomChestContent n = getChestGenBase(item, this, random, wrcc);
                
                if (n != null) {wrcc_array.add(n);}
            }
        }
    ...
    public WeightedRandomChestContent getChestGenBase(Item item, ChestGenHooks chest, Random random, WeightedRandomChestContent original)
    {
        if (item instanceof ItemEnchantedBook)
        {
            ...
            EnchantmentHelper.addRandomEnchantment(random, itemstack, 30, true);
            return new WeightedRandomChestContent(itemstack, original.minStackSize, original.maxStackSize, original.itemWeight);
        }
        return original;
    }
