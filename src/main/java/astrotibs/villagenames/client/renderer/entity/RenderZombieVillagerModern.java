...
//    				& ((profession >= 0 & profession <= 4) | index_of_profession_and_career != -1)
    				)
    		{
...
//    				GeneralConfig.profession_concat_career_a.indexOf((new StringBuilder()).append(profession).append(careerID).toString()));
			...
			if (
//					profession >= 0
//					// Is vanilla OR is a modular type
//					& (profession <= 4 | index_of_profession_and_career!=-1)
//					&
					!zombie.isInvisible()
					)
			{
	...
	public class LayerZombieVillagerProfession implements LayerRenderer<EntityZombie>
	{
	    ...
	    public void doRenderLayer(EntityZombie zombie, float p_177141_2_, float p_177141_3_, float partialTicks, float p_177141_5_, float p_177141_6_, float p_177141_7_, float scale)
	    {
			...
			{
				if (profession <= 4) // There is no official Profession 5!
				{
					// Career skins
					if (GeneralConfig.modernVillagerSkins)
					{
						
			        	int career = ieep.getCareer();
...
//					}
	...
	public class LayerZombieVillagerProfessionLevel implements LayerRenderer<EntityZombie>
	{
	    ...
	    public void doRenderLayer(EntityZombie zombie, float p_177141_2_, float p_177141_3_, float partialTicks, float p_177141_5_, float p_177141_6_, float p_177141_7_, float scale)
	    {
			...
			if (profession >= 0 & seeBiomeTypeForThis & !zombie.isInvisible())
			{
				// Biome type skins
				if (GeneralConfig.modernVillagerSkins)
				{
					//LogHelper.info((ExtendedVillager.get(villager)).getProfessionLevel());
					switch (ieep.getProfessionLevel())
					{
