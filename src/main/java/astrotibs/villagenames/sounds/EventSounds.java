	...
	public void onLivingUpdateEvent(LivingUpdateEvent event) {
		...
		if (!event.getEntityLiving().world.isRemote) {
			...
			if (
		        ...
		        for (Object entity : event.getEntity().world.playerEntities) {
				...
				for (EntityPlayerMP entityplayermp : allPlayers) {
		            ...
		            World world = event.getEntity().world;//player.world;
		                	...
		                	if (
		                		...
		                		if (
		        		    		...
		        		    		if (entityplayermp == listPlayersInRange.get(0)) {
		        		    			entityplayermp.world.playSound(
		        		    					(EntityPlayer)null,
		        		    					  eyeX + 0.5
		        		    					, eyeY + 0.5
		        		    					, eyeZ + 0.5
		...
		if ( !event.getEntityPlayer().world.isRemote ) {
			...
			if (
				...
				/*
					...
					event.getEntityPlayer().world.playSoundEffect(
		...
		if ( 
			...
			for (int nudge = -1; nudge <= 1; nudge++) { // The frame block might not be the center of a side, so check each of the three possible offsets
				...
				if (eyedCorrectFramePieces==11 && uneyedCorrectFramePieces==1) {
					...
					// Portal appearance cancellation has been fixed.
					return new Integer[]{
