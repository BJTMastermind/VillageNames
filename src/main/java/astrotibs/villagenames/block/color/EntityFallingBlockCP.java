...
public class EntityFallingBlockCP extends EntityFallingBlock
{
    ...
    public EntityFallingBlockCP(World worldIn)
    {
        super(worldIn);
    }

    public EntityFallingBlockCP(World worldIn, double x, double y, double z, IBlockState fallingBlockState)
    {
        super(worldIn);
            ...
            if (!this.worldObj.isRemote)
            {
                ...
                if (d0 > 0.0D)
                {
                    ...
                    for (int jOff=0; jOff <=Math.max(0, motionY<-1.0 ? 1 : 0 ); jOff++) { // If it's moving downward faster than a threshold speed: 1 in this case
                    	if(this.worldObj.getBlockState( new BlockPos( blockpos1.getX(), blockpos1.getY()-jOff, blockpos1.getZ() ) ).getBlock().getMaterial(null) == Material.WATER )
                    	{
                    		this.setDead();
                    		
                    		this.worldObj.setBlockState(new BlockPos( blockpos1.getX(), blockpos1.getY()-jOff, blockpos1.getZ() ), this.fallTile, 2);
                    		break;
                    	}
                    }
                }
