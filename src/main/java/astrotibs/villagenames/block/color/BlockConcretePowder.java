...
public class BlockConcretePowder extends BlockFalling implements IMetaBlockName {
    ...
    //change canFallInto to canFallThrough
    //change getDamageValue to getMetaFromState
    //change onNeighborBlockChange to neighborChanged
    ...
    protected boolean tryTouchWater(World worldIn, BlockPos pos)//, Block block)
    {
    	int xo = pos.getX(); int yo = pos.getY(); int zo = pos.getZ();
        boolean flag = false;
        
        for (int side = 1; side < 6; side++)//EnumFacing enumfacing : EnumFacing.values())
        {
        ...
        if (flag)
        {
        	int meta = worldIn.getBlockState(pos).getBlock().getMetaFromState( worldIn.getBlockState(pos) );//.getDamageValue(worldIn, pos);
        	worldIn.setBlockState(pos, ModBlocksVN.blockConcrete.getStateFromMeta( meta ), 2);//.setBlock(pos.getX(), pos.getY(), pos.getZ(), concreteOut, meta, 2);
