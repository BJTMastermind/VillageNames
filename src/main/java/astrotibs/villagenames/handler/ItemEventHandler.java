...
public class ItemEventHandler {
	...
	public String nitwitProfession = GeneralConfigHandler.nitwitProfession;
	
	// Mod name stuff
	public static String[] otherModProfessions = OtherModConfigHandler.otherModProfessions;
	...
	public void onEntityInteract(EntityInteract event) {
		...
		if (event.getEntityPlayer() instanceof EntityPlayer
			...
			//player.sendMessage(new TextComponentString( "This is a "+event.getTarget().getClass().toString() +" right here." ) );
			...
			if (
				...
				if (
					...
					if ( villagerAge < 0 ) {
	            		...
	            		Village villageNearVillager = event.getEntityPlayer().world.villageCollectionObj.getNearestVillage(new BlockPos(villagerX, villagerY, villagerZ), villageRadiusBuffer);//.findNearestVillage((int)villagerX, (int)villagerY, (int)villagerZ, villageRadiusBuffer);
	            		...
	            		if (villageNearVillager != null && player.dimension==0) {
	            			...
	            			if (playerRep < 0) {
		    			        ...
		    			        while(itr.hasNext()) {
		    			            ...
		    			            Village villageNearSign = event.getEntityPlayer().world.villageCollectionObj.getNearestVillage(new BlockPos(townX, townY, townZ), villageRadiusBuffer);//.findNearestVillage((int)townX, (int)townY, (int)townZ, villageRadiusBuffer);
		    			            
		    			            if (villageNearSign == villageNearVillager) {
		    			            	signLocated = true;
		    			            	if (!event.getEntityPlayer().world.isRemote) {
		    			            		
		    			            		if (!player.isSneaking() && villagerProfession!=5) {crouchBookToggle = true;}
				...
				// ...or NAME the villager if he's not named. Open the GUI too.
					...
					nitwitProfession = nitwitProfession.replaceAll("\\(", "");
					nitwitProfession = nitwitProfession.replaceAll("\\)", "");
					nitwitProfession = nitwitProfession.trim();
					if (customName == "") { // Empty name field is represented by "", and not: null
						...
						else if ( // Nitwit Villager has a career tag that's different from the one he should have
								customName.indexOf("(")!=-1
								&& addJobToName
								&& villagerProfession==5) {
									if (!customName.substring(customName.indexOf("(")+1, customName.indexOf(")")).equals(nitwitProfession) ) {
										String careerTag = NameGenerator.getCareerTag(villagerProfession, villagerCareer, nitwitProfession);
										if (careerTag.length()>2) {
											String nameRoot = customName+" "+careerTag;
											villager.setCustomNameTag( customName.substring(0, customName.indexOf("(")-1) + " " + careerTag );
										}
										else {
											villager.setCustomNameTag( customName.substring(0, customName.indexOf("(")-1) );
										}
									}
								}
	...
	private void writeNewVillageBook(EntityInteract event, NBTTagCompound tagList, Village villageNearVillager, int villagerProfession, int villagerCareer) {
		...
		if ( (customName != "" && customName != null) || !nameVillagers ) {
			...
			BlockPos enderEyeBlockPos = ((WorldServer)worldIn).getChunkProvider().getStrongholdGen(worldIn, "Stronghold", new BlockPos(playerIn), false);
	...
	private String[] tryGetStructureName(String nearbyStructure, int[] structureCoords, EntityInteract event) {
		...
		else if (nearbyStructure.equals("GC_AbandonedBase")) {
			data = VNWorldDataAbandonedBase.forWorld(event.getEntity().world);
		}
	...
	private void repairPlayerReputation(EntityPlayer player, Entity target, World world) {
		...
		Village villageNearVillager = target.world.villageCollectionObj.getNearestVillage(new BlockPos(villagerX, villagerY, villagerZ), villageRadiusBuffer);
		if (villageNearVillager != null && player.dimension==0) {
			...
			while(itr.hasNext()) {
				...
				Village villageNearSign = player.world.villageCollectionObj.getNearestVillage(new BlockPos(townX, townY, townZ), villageRadiusBuffer);//.findNearestVillage((int)townX, (int)townY, (int)townZ, villageRadiusBuffer);
