...
public class ItemEventHandler {
	...
	//String nitwitProfession = GeneralConfigHandler.nitwitProfession;//.config.getString("nitwitProfession", "Profession/Career", "", "The career displayed for a Nitwit");
	...
	boolean useStructureChunkSearch = true;
	...
	public void headOffRepWiping(PlayerEvent.PlayerLoggedInEvent load) {
		World world = load.player.worldObj;//.getWorld();
		if (!world.isRemote && repairPlayerRep) {
			...
			List vlist = world.villageCollectionObj.getVillageList();
			Iterator vitr = vlist.iterator();

			// Get list of villagenames.dat villages
			VNWorldDataVillage data = VNWorldDataVillage.forWorld(world); //event.entity.worldObj
	...
	public void AttackVillager(AttackEntityEvent aEEvent) {
		if (repairPlayerRep && 
			...
			if (
					!aEEvent.getEntityPlayer().worldObj.isRemote &&
	...
	public void onEntityInteract(EntityInteract event) {
		...
		if (event.getEntityPlayer() instanceof EntityPlayer
			&& !event.getEntityPlayer().worldObj.isRemote) {
			...
			//player.addChatComponentMessage(new TextComponentString( "This is a "+event.getTarget().toString() +" right here." ) );
			...
			if (
				...
				if (
					...
					if ( villagerAge < 0 ) {
	            		...
	            		if (babyMessageToggle) {
	            			player.addChatComponentMessage(new TextComponentString( babyCantHelpString() ) );
	            		}
					}
	            		...
	            		Village villageNearVillager = event.getEntityPlayer().worldObj.villageCollectionObj.getNearestVillage(new BlockPos(villagerX, villagerY, villagerZ), villageRadiusBuffer);//.findNearestVillage((int)villagerX, (int)villagerY, (int)villagerZ, villageRadiusBuffer);
	            		...
	            		if (villageNearVillager != null && player.dimension==0) {
	            			...
	            			if (playerRep < 0) {
	            				...
	            				player.addChatComponentMessage(new TextComponentString( "The villager does not trust you." ) );
	            			}
	            			else {
	            				...
	            				if (itemstack.getItem() == Items.EMERALD) { // Trade with emeralds only
	            					if (emeralds >= emeraldReqiured+1) {
	            						...
	            						player.addChatComponentMessage(new TextComponentString( "Need more emeralds." ) );
	            					}
	            					...
	            					if (emeralds >= emeraldReqiured) {
	            						if (goldIngots >= goldRequired) {
	            							...
	            							player.addChatComponentMessage(new TextComponentString( "Need more gold ingots." ) );
	            						}
	            						...
	            						if (goldIngots >= goldRequired) {
	            							...
	            							player.addChatComponentMessage(new TextComponentString( "Need more emeralds and gold ingots." ) );
	            						}
	            					}
	            					...
	            					if (emeralds >= emeraldReqiured) {
	            						if (ironIngots >= ironRequired) {
	            							...
	            							player.addChatComponentMessage(new TextComponentString( "Need more iron ingots." ) );
	            						}
	            						...
	            						if (ironIngots >= ironRequired) {
	            							...
	            							player.addChatComponentMessage(new TextComponentString( "Need more emeralds and iron ingots." ) );
	            						}
	            					}
	            			...
	            			player.addChatComponentMessage(new TextComponentString( "The villager seems " + cantHelpAdjective + ", and glances around." ) );
	            		}
				...
				// I'm hoping what this does is prevent you from naming a villager with a name tag. We'll see.
					...
					player.addChatComponentMessage(new TextComponentString("That's not this villager's name..."));
					event.setCanceled(true);
					...
					if ( villagerAge < 0 ) {
	            		...
	            		if (villageNearVillager != null && player.dimension==0) {
	            			...
	            			if (playerRep < 0) {
		    					...
		    					VNWorldDataVillage data = VNWorldDataVillage.forWorld(event.getWorld()); //event.entity.worldObj
		    			        ...
		    			        while(itr.hasNext()) {
		    			            ...
		    			            Village villageNearSign = event.getEntityPlayer().worldObj.villageCollectionObj.getNearestVillage(new BlockPos(townX, townY, townZ), villageRadiusBuffer);//.findNearestVillage((int)townX, (int)townY, (int)townZ, villageRadiusBuffer);
	            			...
	            			String cantHelpAdjective = cantHelpArray[random.nextInt(cantHelpArray.length)];
	            			player.addChatComponentMessage(new TextComponentString( "The villager gives you a " + cantHelpAdjective + " look." ) );
	            		}
				...
				if (
					...
					player.addChatComponentMessage(new TextComponentString("That name is just an epithet."));
								...
								if (nbtbase.getId() == 10) {
									...
									try {
										...
										if (
											...
											String authorName = customName;//player.getDisplayName();
	...
	private void writeNewVillageBook(EntityInteract event, NBTTagCompound tagList, Village villageNearVillager, int villagerProfession, int villagerCareer) {
		...
		NBTTagCompound compound = new NBTTagCompound();
		villager.writeEntityToNBT(compound);
		String customName = event.getTarget().getCustomNameTag();
		...
		if ( (customName != "" && customName != null) || !nameVillagers ) {
			...
			BlockPos enderEyeBlockPos = ((WorldServer)worldIn).getChunkProvider().getStrongholdGen(worldIn, "Stronghold", new BlockPos(playerIn));
			...
			switch (villagerMappedProfession) {
				...
				case 1: // Villager is a Librarian. Find a Stronghold or a Woodland Mansion.
					nearestStrongholdXYZ = new int[]{enderEyeBlockPos.getX(),enderEyeBlockPos.getY(),enderEyeBlockPos.getZ()};
	...
	private int[] nearestStructureLoc(String structureName, EntityInteract event, double xOffset, double zOffset){
		...
		Map<String, BlockPos> nearbyStructures = StructureRegistry.instance.getNearestStructures((WorldServer)event.getEntityPlayer().worldObj, (int)(event.getTarget().posX + xOffset), (int)event.getTarget().posY, (int)(event.getTarget().posZ + zOffset) );
	...
	private String writeStructureHintPage(String nearbyStructure, int[] structureCoords, int villagerProfession, double[] villageCoords, int villageRadius, EntityInteract event ) {
		...
		if (structureNameArray[0]==null && structureNameArray[1]==null && structureNameArray[2]==null) {
			...
			if (nearbyStructure.equals("Village")) {
				VNWorldDataVillage data = VNWorldDataVillage.forWorld(event.getEntity().worldObj);
				...
				VNWorldDataMineshaft data = VNWorldDataMineshaft.forWorld(event.getEntity().worldObj);
				...
				VNWorldDataStronghold data = VNWorldDataStronghold.forWorld(event.getEntity().worldObj);
				...
				VNWorldDataTemple data = VNWorldDataTemple.forWorld(event.getEntity().worldObj);
				...
				VNWorldDataMonument data = VNWorldDataMonument.forWorld(event.getEntity().worldObj);
			...
			/*
				...
				VNWorldDataMansion data = VNWorldDataMansion.forWorld(event.getEntity().worldObj);
	...
	private String[] tryGetStructureName(String nearbyStructure, int[] structureCoords, EntityInteract event) {
		...
		if (nearbyStructure.equals("Village")) {
			data = VNWorldDataVillage.forWorld(event.getEntity().worldObj);
		}
		else if (nearbyStructure.equals("Mineshaft")) {
			data = VNWorldDataMineshaft.forWorld(event.getEntity().worldObj);
		}
		else if (nearbyStructure.equals("Stronghold")) {
			data = VNWorldDataStronghold.forWorld(event.getEntity().worldObj);
		}
		else if (nearbyStructure.equals("Temple")) {
			data = VNWorldDataTemple.forWorld(event.getEntity().worldObj);
		}
		else if (nearbyStructure.equals("Monument")) {
			data = VNWorldDataMonument.forWorld(event.getEntity().worldObj);
		}
		else if (nearbyStructure.equals("Mansion")) {
			data = VNWorldDataMansion.forWorld(event.getEntity().worldObj);
		}
	...
	private int[] getStructureFromChunk(String structureName, EntityInteract event) {
		...
		MapGenStructureData structureData;
		World worldIn = event.getEntityPlayer().worldObj;
	...
	private void repairPlayerReputation(EntityPlayer player, Entity target, World world) {
		...
		Village villageNearVillager = target.worldObj.villageCollectionObj.getNearestVillage(new BlockPos(villagerX, villagerY, villagerZ), villageRadiusBuffer);
		if (villageNearVillager != null && player.dimension==0) {
			...
			while(itr.hasNext()) {
				...
				Village villageNearSign = player.worldObj.villageCollectionObj.getNearestVillage(new BlockPos(townX, townY, townZ), villageRadiusBuffer);//.findNearestVillage((int)townX, (int)townY, (int)townZ, villageRadiusBuffer);
			...
			if ( 
				...
				specificVillageTag.setTag("playerReps", nbttaglist);//.setInteger("testVal", 0);
