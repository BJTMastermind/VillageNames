...
public class ItemEventHandler {
	...
	public void onEntityInteract(EntityInteractEvent event) {
		...
		if (event.entity instanceof EntityPlayer
			...
			if (
				 ...
				 /*
				  * THIS STUFF DETECTS NEARBY DETECTABLE STRUCTURES
				  */
				Map<String, BlockPos> nearbyStructures = StructureRegistry.instance.getNearestStructures((WorldServer)event.entityPlayer.worldObj, (int)(event.target.posX), (int)event.target.posY, (int)(event.target.posZ) );
				...
				String customName = event.target.getCustomNameTag();
				...
				if (
					...
					if ( villagerAge < 0 ) {
	            		...
	            		Village villageNearVillager = event.entityPlayer.worldObj.villageCollectionObj.getNearestVillage(new BlockPos(villagerX, villagerY, villagerZ), villageRadiusBuffer);
	            		
	            		if (villageNearVillager != null && player.dimension==0) {
	            			int playerRep = villageNearVillager.getReputationForPlayer(event.entityPlayer.getDisplayNameString());
	            			if (playerRep < 0) {
	            				...
	            				if (itemstack.getItem() == Items.emerald) { // Trade with emeralds only
	            					if (emeralds >= emeraldReqiured+1) {
		            			        ...
		            			        if(random.nextInt(2)==0) {villageNearVillager.setReputationForPlayer(event.entityPlayer.getDisplayNameString(), playerRep+1);}
	            					}
	            					else {
				...
				// Now either create a Village Book if you're holding a normal book...
					...
					if ( villagerAge < 0 ) {
	            		...
	            		// Finds the nearest village, but only if the given coordinates are within its bounding radius plus the buffer
	            		Village villageNearVillager = event.entityPlayer.worldObj.villageCollectionObj.getNearestVillage(new BlockPos(villagerX, villagerY, villagerZ), villageRadiusBuffer);//.findNearestVillage((int)villagerX, (int)villagerY, (int)villagerZ, villageRadiusBuffer);
	            		...
	            		if (villageNearVillager != null && player.dimension==0) {
	            			...
	            			if (playerRep < 0) {
		    			        ...
		    			        while(itr.hasNext()) {
		    			            ...
		    			            Village villageNearSign = event.entityPlayer.worldObj.villageCollectionObj.getNearestVillage(new BlockPos(townX, townY, townZ), villageRadiusBuffer);
				...
				// Create Alien Village book
					...
					if ( villagerAge < 0 ) {
						...
						try {
							structureData = (MapGenStructureData)worldIn.getPerWorldStorage().loadData(MapGenStructureData.class, s);
	...
	private int[] nearestStructureLoc(String structureName, EntityInteractEvent event, double xOffset, double zOffset){
		...
		Map<String, BlockPos> nearbyStructures = StructureRegistry.instance.getNearestStructures((WorldServer)event.entityPlayer.worldObj, (int)(event.target.posX + xOffset), (int)event.target.posY, (int)(event.target.posZ + zOffset) );
