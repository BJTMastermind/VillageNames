...
public class WellDecorateEvent {
	...
	public void onPopulating(PopulateChunkEvent.Post event) {
		...
		if (
				event.hasVillageGenerated && event.world.provider.getDimensionId() == 0 && !VillageGeneratorConfigHandler.newVillageGenerator
                                ...
                                if (listWater.size() == 3 && listWater2.size() == 3 && listWater3.size() == 3 && listOpaque.size() == 5) {//found 3 water blocks AND 5 opaque blocks surrounding one water block on THREE levels, assuming this is a village well
                                	...
                                	if (1-Math.abs(isWellCorner/2-1)+((isWellCorner-1)%2)*2 == signLocation) {
                                		...
                                		if ( event.world.getBlockState( new BlockPos(x+signXOffset, y+2, z+signZOffset) ).getBlock().isAir(event.world, new BlockPos(x+signXOffset, y+2, z+signZOffset)) ||
                                    		...
                                    		try
                                    		{
                                    			structureData = (MapGenStructureData)event.world.getPerWorldStorage().loadData(MapGenStructureData.class, "Village");
                                    		...
                                    		catch (Exception e) // Village.dat does not exist
                                    		{
                                    			try
                                        		{
                                        			structureData = (MapGenStructureData)event.world.getPerWorldStorage().loadData(MapGenStructureData.class, "OTGVillage");
                                        			nbttagcompound = structureData.getTagCompound();
                                        		}
                                    		...
                                    		if (GeneralConfig.wellDecorations && GeneralConfig.nameSign) {
                                    			...
                                    			BlockPos signBasePos = new BlockPos(x+signXOffset, y+1, z+signZOffset);
                                    			
                                    			event.world.setBlockState(signBasePos, Blocks.cobblestone_wall.getDefaultState() );
                                    		...
                                    		if (GeneralConfig.wellDecorations && GeneralConfig.villageBanners && signLocation!=bannerLocation) {
                                    			...
                                    			event.world.setBlockState(bannerBasePos, Blocks.cobblestone_wall.getDefaultState() );
                                    			event.world.setBlockState(bannerBasePos.up(), Blocks.standing_banner.getStateFromMeta(bannerOrientation) );
                                    			
                                    			TileEntity tilebanner = new TileEntityBanner();
                                    			((TileEntityBanner) tilebanner).setItemValues(villageBanner);
                                        		event.world.setTileEntity(bannerBasePos.up(), tilebanner);
                                    		}
                                    		...
                                    		if (GeneralConfig.wellDecorations && GeneralConfig.nameSign) event.world.setTileEntity(new BlockPos(x+signXOffset, y+2, z+signZOffset), signContents);
		...
		if (
				...
				&& event.world.getBiomeGenForCoords( new BlockPos(event.chunkX * 16 + 8, 0, event.chunkZ * 16 + 8) ) == BiomeGenBase.swampland
				) {
	...
	public void searchHutAndReplacePot(PopulateChunkEvent.Post event, int buffer)
    {
		...
		if (
			...
			try {
        		...
        		try
        		{
        			structureData = (MapGenStructureData)event.world.getPerWorldStorage().loadData(MapGenStructureData.class, "Temple");
        			nbttagcompound = structureData.getTagCompound();
        		}
        		catch (Exception e)
        		{
        			try
            		{
            			structureData = (MapGenStructureData)event.world.getPerWorldStorage().loadData(MapGenStructureData.class, "OTGTemple");
            			nbttagcompound = structureData.getTagCompound();
            		}
            		catch (Exception e1) {}
        		}
								...
								for (int k = 50; k <= 80; k++) {
									for (int i=-16; i < 16; i++) {
										for (int j=-16; j < 16; j++) {
											...
											potPos = new BlockPos((event.chunkX << 4)+i, k, (event.chunkZ << 4)+j);
											...
											if (blocktoscan == Blocks.flower_pot)
											{
												...
												if (potmeta != 7 )
												{
													...
													event.world.setTileEntity(potPos, flowerPotWithRedMushroom);
