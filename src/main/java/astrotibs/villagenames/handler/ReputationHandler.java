...
public class ReputationHandler {
    ...
    public static int getVNReputationForPlayer(EntityPlayerMP player, String villageTopTag, Village village)
    {
		...
		try
		{
			structureData = (MapGenStructureData)player.worldObj.getPerWorldStorage().loadData(MapGenStructureData.class, "Village");
		...
		catch (Exception e) // Village.dat does not exist
		{
			try
    		{
    			structureData = (MapGenStructureData)player.worldObj.getPerWorldStorage().loadData(MapGenStructureData.class, "OTGVillage");
    			nbttagcompound = structureData.getTagCompound();
    		}
    	...
    	if (villageTag!=null) { // Otherwise, the game crashes when in an artificial village
        	...
        	if (villageTag.hasKey(repKey)) {
        		...
        		if (playerReps.hasKey(player.getUniqueID().toString())) {
        			...
        			if (village==null) { // There is no village in the collectionObj--presumably, it was annihilated. Return the VN rep.
        				...
        				int collectionRep = village.getReputationForPlayer(player.getDisplayNameString());
        				if (PrevVNPlayerRep == collectionRep) { // They match.
    						...
    						if (villageTag.getBoolean(collectedKey)) { // This village was previously reported as "collected"
    							...
    							village.setReputationForPlayer(player.getDisplayNameString(), PrevVNPlayerRep-collectionRep);
    							VNPlayerRep = PrevVNPlayerRep; 
    		...
    		if (returnDefaultRep) {
    			...
    			if (village!=null) {
    				...
    				VNPlayerRep = village.getReputationForPlayer(player.getDisplayNameString());
    		...
    		try {
    			nullVillageTagRep = village.getReputationForPlayer(player.getDisplayNameString());
    		}
