...
public class ReputationHandler {
    ...
    public static int getVNReputationForPlayer(EntityPlayerMP player, String villageTopTag, Village village)
    {
		...
		try
		{
			structureData = (MapGenStructureData)player.world.getPerWorldStorage().getOrLoadData(MapGenStructureData.class, "Village");
			nbttagcompound = structureData.getTagCompound();
		}
		catch (Exception e) // Village.dat does not exist
		{
			try
    		{
    			structureData = (MapGenStructureData)player.world.getPerWorldStorage().getOrLoadData(MapGenStructureData.class, "OTGVillage");
    			nbttagcompound = structureData.getTagCompound();
    		}
    		catch (Exception e1) {} // OTGVillage.dat does not exist
		}
    	...
    	if (villageTag!=null) { // Otherwise, the game crashes when in an artificial village
    		...
    		if (villageTag.hasKey(repTagKey)) {
        		...
        		if (playerReps.hasKey(player.getUniqueID().toString())) {
        			...
        			if (village==null) { // There is no village in the collectionObj--presumably, it was annihilated. Return the VN rep.
        				...
        				int collectionRep = village.getPlayerReputation(player.getUniqueID());//village.getPlayerReputation(player.getDisplayNameString());
        				...
        				else if (collectionRep!=0) { // They do not match.
    						if (
    							...
    							village.modifyPlayerReputation(player.getUniqueID(), PrevVNPlayerRep-collectionRep);//village.modifyPlayerReputation(player.getDisplayNameString(), PrevVNPlayerRep-collectionRep);
    							VNPlayerRep = PrevVNPlayerRep; 
    						}
        				}
        			}
    		...
    		if (returnDefaultRep) {
    			...
    			if (village!=null) {
    				...
    				VNPlayerRep = village.getPlayerReputation(player.getUniqueID());//village.getPlayerReputation(player.getDisplayNameString());
    		...
    		try {
    			nullVillageTagRep = village.getPlayerReputation(player.getUniqueID());//village.getPlayerReputation(player.getDisplayNameString());
    		}
