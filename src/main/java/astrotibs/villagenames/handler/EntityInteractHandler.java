...
public class EntityInteractHandler {
	...
	public void onEntityInteract(EntityInteract event)
	{
		...
		if (
			...
			{
				try {
		    		int indexofmodprof = GeneralConfig.modProfessionMapping_map.get("IDs").indexOf(((EntityVillager)target).getProfessionForge().getRegistryName().toString());
					...
					&& itemstackMain.getItem() == Items.NAME_TAG)
			{
				 ...
				 * summon Horse ~ ~ ~ {Tame:1, Owner:"AstroTibs"}
