...
public class SpawnNamingHandler {
	...
	//public void onPopulating(EntityConstructing event) {
		...
		if (event.getEntity() instanceof EntityLiving
				...
				&& !event.getEntity().worldObj.isRemote) { // Only looks at living entities in order to reduce grind
			...
			if (mappedNamesAutomatic.get("ClassPaths").contains(entityClassPath) ) {
				...
				if (
					...
					//TODO - Change the version in the warning.
					LogHelper.warn("Entity " + entity.getClass().toString().substring(6) + " is a boss. Unfortunately, custom names do not display on the vanilla boss bar overlay in 1.9.");
				}
			}
		}
	}
