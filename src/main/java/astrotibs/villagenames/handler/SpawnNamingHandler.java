...
public class SpawnNamingHandler {
	...
	//public void onPopulating(EntityConstructing event) {
		...
		//if (event.getEntity().dimension==1 && !event.getEntity().worldObj.isRemote) {LogHelper.info("End entity " + event.getEntity() );}
		...
		if (
				...
				//&& !event.getEntity().getPosition().equals(new BlockPos(0,128,0)) 
			...
			if (event.getEntity() instanceof EntityGuardian) {
				if ( ((EntityGuardian) event.getEntity()).isElder() ) { // Reference "Elder" guardians using the below string. Reference ordinary guardians as EntityGuardian
			...
			if (mappedNamesAutomatic.get("ClassPaths").contains(entityClassPath) ) {
					...
					if ( (entity.getCustomNameTag().trim().equals("") || entity.getCustomNameTag().trim() == null
							||
							 (entityClassPath.equals("net.daveyx0.primitivemobs.entity.passive.EntityTravelingMerchant")
							   && entity.getCustomNameTag().trim().equals("Traveling Merchant"))
				...
				if (
					...
					if (entity instanceof EntityDragon) {
						LogHelper.warn("Ender Dragon custom names do not work properly. Additionally, custom names do not display on the vanilla boss bar overlay in 1.10.");
					}
					else {
						LogHelper.warn("Entity " + entity.getClass().toString().substring(6) + " is a boss. Unfortunately, custom names do not display on the vanilla boss bar overlay in 1.10.");
					}
