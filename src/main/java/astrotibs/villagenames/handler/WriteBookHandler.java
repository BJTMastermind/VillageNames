...
public class WriteBookHandler {
	...
	public static void targetWriteNewVillageBook (
			...
			boolean playerIsInVillage, Village villageYouAreIn, EntityInteract event,
			int targetProfession, int targetCareer, String targetPName,
		...
		if (population > -1 && !(Loader.isModLoaded("toroquest") && GeneralConfig.TQVillageNames) ) {bookContents += "\n\u00a7rPopulation: \u00a7l" + population;} 
		...
		// Vanilla structures
		     if (bookType.toLowerCase().trim().equals("mineshaft")) {book = new ItemStack(ModItems.MINESHAFT_BOOK);}
		else if (bookType.toLowerCase().trim().equals("temple")) {book = new ItemStack(ModItems.TEMPLE_BOOK);}
		else if (bookType.toLowerCase().trim().equals("monument")) {book = new ItemStack(ModItems.MONUMENT_BOOK);}
		else if (bookType.toLowerCase().trim().equals("fortress")) {book = new ItemStack(ModItems.FORTRESS_BOOK);}
		else if (bookType.toLowerCase().trim().equals("stronghold")) {book = new ItemStack(ModItems.STRONGHOLD_BOOK);}
		else if (bookType.toLowerCase().trim().equals("endcity")) {book = new ItemStack(ModItems.END_CITY_BOOK);}
		else if (bookType.toLowerCase().trim().equals("mansion")) {book = new ItemStack(ModItems.MANSION_BOOK);}
		
		// Mod structures     
		else if (bookType.toLowerCase().trim().equals("moonvillage")) {book = new ItemStack(ModItems.MOON_VILLAGE_BOOK);}
		else if (bookType.toLowerCase().trim().equals("fronosvillage")) {book = new ItemStack(ModItems.FRONOS_VILLAGE_BOOK);}
		else if (bookType.toLowerCase().trim().equals("koentusvillage")) {book = new ItemStack(ModItems.KOENTUS_VILLAGE_BOOK);}
		else if (bookType.toLowerCase().trim().equals("nibiruvillage")) {book = new ItemStack(ModItems.NIBIRU_VILLAGE_BOOK);}
		else if (bookType.toLowerCase().trim().equals("abandonedbase")) {book = new ItemStack(ModItems.ABANDONED_BASE_BOOK);}
		     
		// New temples
		else if (bookType.toLowerCase().trim().equals("jungletemple")) {book = new ItemStack(ModItems.JUNGLE_TEMPLE_BOOK);}
		else if (bookType.toLowerCase().trim().equals("desertpyramid")) {book = new ItemStack(ModItems.DESERT_PYRAMID_BOOK);}
		else if (bookType.toLowerCase().trim().equals("swamphut")) {book = new ItemStack(ModItems.SWAMP_HUT_BOOK);}
		else if (bookType.toLowerCase().trim().equals("igloo")) {book = new ItemStack(ModItems.IGLOO_BOOK);}
		
		else {book = new ItemStack(ModItems.VILLAGE_BOOK);} // By default, or if the type was misspelled, use a Village book.

		List<String> pages = new ArrayList<String>();
		
		if (book.getTagCompound() == null) {book.setTagCompound(new NBTTagCompound());} // Priming the book to receive information
		book.getTagCompound().setString("title", (namePrefix + " " + nameRoot + " " + nameSuffix).trim() ); // Set the title
		// Set the author
		if (author!=null && !author.equals("")) {
			try { book.getTagCompound().setString("author", author.indexOf("(")!=-1 ? author.substring(0, author.indexOf("(")).trim() : author ); }
		...
		else {book.getTagCompound().setString("author", "");}
        ...
        if (target!=null) {
        		...
        		try {
	            	String structureHintPageText = makeSecondPage(
	            			...
	            			locX, locY, locZ,
	            			playerRep, targetProfession, targetCareer, targetPName, targetTradeCount );
        ...
        if (!player.capabilities.isCreativeMode) {player.inventory.clearMatchingItems(Items.BOOK, -1, 1, null);}
        ...
        if (target == null // There was no "target" (i.e. Villager) to create the book
        	...
        	try {
        		if (!((EntityPlayerMP) player).getStatFile().hasAchievementUnlocked(VillageNames.ghosttown)) {
        			//player.triggerAchievement(VillageNames.ghosttown);
        			player.addStat(VillageNames.ghosttown);
	...
	public static void codexWriteNewVillageBook (
		...
		// Consume Codex
		if (!player.capabilities.isCreativeMode) { player.inventory.clearMatchingItems(ModItems.CODEX, -1, 1, null); }
	...
	public static String makeSecondPage(EntityInteract event, String targetClassPath, Village villageNearVillager,
    	...
    	List vlist = event.getEntityPlayer().world.villageCollectionObj.getVillageList();
    	...
    	while (vitr.hasNext()) {
    		...
    		int vx = element.getCenter().getX();
    		int vy = element.getCenter().getY();
    		int vz = element.getCenter().getZ();
    		dx = vx - villageNearVillager.getCenter().getX();
    		dy = vy - villageNearVillager.getCenter().getY();
    		dz = vz - villageNearVillager.getCenter().getZ();
		...
		try
		{
			structureData = (MapGenStructureData)event.getEntityPlayer().world.getPerWorldStorage().getOrLoadData(MapGenStructureData.class, "Village");
		...
		catch (Exception e) // Village.dat does not exist
		{
			try
    		{
    			structureData = (MapGenStructureData)event.getEntityPlayer().world.getPerWorldStorage().getOrLoadData(MapGenStructureData.class, "OTGVillage");
    			nbttagcompound = structureData.getTagCompound();
    		}
		...
		while (itr.hasNext()) {
			...
			if (nbtbase.getId() == 10) {
				...
				try {
					...
					int vx = villageNearVillager.getCenter().getX();
					int vy = villageNearVillager.getCenter().getY();
					int vz = villageNearVillager.getCenter().getZ();
    	...
    	// Special Nitwit handler removed because a modded one is not needed.
	 ...
	 */
	private static int[] nearestStructureLoc(String structureName, EntityInteract event, double xOffset, double zOffset, boolean includeThis) {
		
		// This is p_180513_4_ for ChunkProviderServer.getStrongholdGen(World worldIn, String structureName, BlockPos position, boolean p_180513_4_)
		...
		BlockPos pos = ((WorldServer)event.getEntityPlayer().world).getChunkProvider().getStrongholdGen(event.getEntityPlayer().world, structureName, new BlockPos(event.getEntityPlayer()), getStrongholdGenBool);
	 ...
	 * Used to find the nearest structure to the target entity by structure name. Returns [0,0,0] if none found.
	 */
	private static int[] nearestStructureLoc(String structureName, EntityInteract event){
	...
	private static int[] nearestStructureLoc(String structureName, EntityInteract event, double xOffset, double zOffset, boolean includeThis) {
		...
		Map<String, BlockPos> nearbyStructures = StructureRegistry.instance.getNearestStructures(
				(WorldServer)event.getEntityPlayer().world,
				(int)(event.getTarget().posX + xOffset),
				(int) event.getTarget().posY,
				(int)(event.getTarget().posZ + zOffset)
				);
		...
		for (Map.Entry<String, BlockPos> e : nearbyStructures.entrySet()) {
			...
			double dx = pos.getX() - event.getTarget().posX;
			double dy = pos.getY() - event.getTarget().posY;
			double dz = pos.getZ() - event.getTarget().posZ;
	 ...
	 */
	/*
	public static int[] nearestStructureLoc(String structureName, EntityInteract event){
		return nearestStructureLoc(structureName, event, 0.0f, 0.0f, false);
	}
	*/
	 ...
	 */
	private static String writeStructureHintPage(String nearbyStructure, int[] structureCoords, int villagerProfession, double[] villageCoords, int villageRadius, EntityInteract event ) {
	 ...
	 */
	private static String[] tryGetStructureName(String nearbyStructure, int[] structureCoords, EntityInteract event) {
		
		// Load in names data
		VNWorldData data= VNWorldDataStructure.forWorld(event.getEntity().world, "villagenames3_"+nearbyStructure, "NamedStructures" );
		
		// .getTagList() will return all the entries under the specific village name.
		NBTTagCompound tagCompound = data.getData();
		
		Set tagmapKeyset = tagCompound.getKeySet(); //Gets the town key list: "x535y80z39"
