...
public class WriteBookHandler {
	...
	public static void targetWriteNewVillageBook (
        ...
        if (target == null // There was no "target" (i.e. Villager) to create the book
        		...
        		&& !player.getServer().getPlayerList().getPlayerAdvancements((EntityPlayerMP)player).getProgress(
        				player.getServer().getAdvancementManager().getAdvancement(new ResourceLocation(Reference.MOD_ID+":ghosttown"))
						).isDone()
        		) {
        	
        	// The if condition was inserted so that your achievement stat will cap out at 1
        	ModTriggers.GHOSTTOWN.trigger( ((EntityPlayerMP) player) );
        	AdvancementReward.allFiveAdvancements( ((EntityPlayerMP) player) );
        	/*
        	try {
