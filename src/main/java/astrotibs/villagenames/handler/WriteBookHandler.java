...
public class WriteBookHandler {
	...
	public static String makeSecondPage(EntityInteractEvent event, String targetClassPath, Village villageNearVillager,
		...
		try
		{
			structureData = (MapGenStructureData)event.entityPlayer.worldObj.getPerWorldStorage().loadData(MapGenStructureData.class, "Village");
		...
		catch (Exception e) // Village.dat does not exist
		{
			try
    		{
    			structureData = (MapGenStructureData)event.entityPlayer.worldObj.getPerWorldStorage().loadData(MapGenStructureData.class, "OTGVillage");
    			nbttagcompound = structureData.getTagCompound();
    		}
    	...
    	try
    	{
    		...
    		int index_of_profession_and_career = Math.max(
    				IDs_concat_careers.indexOf((new StringBuilder()).append(villagerProfession).append(-99).toString()), 
    				IDs_concat_careers.indexOf((new StringBuilder()).append(villagerProfession).append(villagerCareer).toString()));
    		
    		villagerMappedProfession = villagerProfession > 4 && index_of_profession_and_career!= -1 
    				? (Integer)GeneralConfig.modProfessionMapping_map.get("VanillaProfMaps").get(index_of_profession_and_career) : villagerProfession;
		}
    	catch (Exception e)
    	{
	...
	private static int[] nearestStructureLoc(String structureName, EntityInteractEvent event, double xOffset, double zOffset, boolean includeThis) {
		...
		for (Map.Entry<String, BlockPos> e : nearbyStructures.entrySet()) {
			...
			double dx = pos.getX() - event.target.posX;
			double dy = pos.getY() - event.target.posY;
			double dz = pos.getZ() - event.target.posZ;
