...
public class WriteBookHandler {
	...
	public static String makeSecondPage(EntityInteract event, String targetClassPath, Village villageNearVillager,
		...
		try
		{
			structureData = (MapGenStructureData)event.getEntityPlayer().worldObj.getPerWorldStorage().getOrLoadData(MapGenStructureData.class, "Village");
		...
		catch (Exception e) // Village.dat does not exist
		{
			try
    		{
    			structureData = (MapGenStructureData)event.getEntityPlayer().worldObj.getPerWorldStorage().getOrLoadData(MapGenStructureData.class, "OTGVillage");
    			nbttagcompound = structureData.getTagCompound();
    		}
	...
	private static int[] nearestStructureLoc(String structureName, EntityInteract event, double xOffset, double zOffset, boolean includeThis) {
		...
		Map<String, BlockPos> nearbyStructures = StructureRegistry.instance.getNearestStructures(
				(WorldServer)event.getEntityPlayer().worldObj,
	...
	private static String[] tryGetStructureName(String nearbyStructure, int[] structureCoords, EntityInteract event) {
		...
		// Load in names data
		VNWorldData data= VNWorldDataStructure.forWorld(event.getEntity().worldObj, "villagenames3_"+nearbyStructure, "NamedStructures" );
