...
public class AlienVillageConfigHandler
{
	...
	{
		alienvillage_prefix = config.getString("Prefixes", Reference.CATEGORY_ALIEN_VILLAGE_SYLLABLE_POOL, NamePieces.alienvillage_prefix_default,
				"Prefixes that can occur before the core name.").trim().split("\\s*,\\s*");
		
		alienvillage_root_initial = config.getString("Root: Initial", Reference.CATEGORY_ALIEN_VILLAGE_SYLLABLE_POOL, NamePieces.alienvillage_root_initial_default,
				"Core names begin with one of these half-syllables. Use _ to represent a space.").trim().split("\\s*,\\s*");
		
		alienvillage_root_syllables = config.getString("Root: Syllables", Reference.CATEGORY_ALIEN_VILLAGE_SYLLABLE_POOL, NamePieces.alienvillage_root_sylBegin_default,
		...
		alienvillage_root_terminal = config.getString("Root: Terminal", Reference.CATEGORY_ALIEN_VILLAGE_SYLLABLE_POOL, NamePieces.alienvillage_root_terminal_default,
				"Core names end with one of these half-syllables. Use _ to represent a space, and ^ for a blank entry.").trim().split("\\s*,\\s*");
		
		alienvillage_suffix = config.getString("Suffixes", Reference.CATEGORY_ALIEN_VILLAGE_SYLLABLE_POOL, NamePieces.alienvillage_suffix_default,
				"Suffixes that can occur after the core name.").trim().split("\\s*,\\s*");

		
		
		syllable_count_weighting = config.get(Reference.CATEGORY_ALIEN_VILLAGE_SYLLABLE_POOL, "Syllable Count Weighting", NamePieces.alienvillage_syllable_count_weights,
		...
		prefix_chance = config.getFloat("Prefix Chance", Reference.CATEGORY_ALIEN_VILLAGE_SYLLABLE_POOL,
		...
		suffix_chance = config.getFloat("Suffix Chance", Reference.CATEGORY_ALIEN_VILLAGE_SYLLABLE_POOL,
