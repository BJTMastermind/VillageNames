...
public class EndCityConfigHandler
{
	...
	{
		endcity_prefix = config.getStringWithoutDefaultsInComment(Reference.NAME_PREFIXES, Reference.CATEGORY_END_CITY_SYLLABLE_POOL, NamePieces.END_CITY_PREFIX_DEFAULT,
				Reference.DESCRIPTION_PREFIXES).trim().split("\\s*,\\s*");
		
		endcity_root_initial = config.getStringWithoutDefaultsInComment(Reference.NAME_ROOT_INITIAL, Reference.CATEGORY_END_CITY_SYLLABLE_POOL, NamePieces.END_CITY_ROOT_INITIAL_DEFAULT,
				"Core names begin with one of these half-syllables. Use _ to represent a space.").trim().split("\\s*,\\s*");
		
		endcity_root_syllables = config.getStringWithoutDefaultsInComment(Reference.NAME_ROOT_SYLLABLES, Reference.CATEGORY_END_CITY_SYLLABLE_POOL, NamePieces.END_CITY_ROOT_SYL_BEGIN_DEFAULT,
				Reference.DESCRIPTION_ROOT_SYLLABLES).trim().split("\\s*,\\s*");
		
		endcity_root_terminal = config.getStringWithoutDefaultsInComment(Reference.NAME_ROOT_TERMINAL, Reference.CATEGORY_END_CITY_SYLLABLE_POOL, NamePieces.END_CITY_ROOT_TERMINAL_DEFAULT,
				Reference.DESCRIPTION_ROOT_TERMINAL).trim().split("\\s*,\\s*");
		
		endcity_suffix = config.getStringWithoutDefaultsInComment("Suffixes", Reference.CATEGORY_END_CITY_SYLLABLE_POOL, NamePieces.END_CITY_SUFFIX_DEFAULT,
