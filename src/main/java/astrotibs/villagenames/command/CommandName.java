...
public class CommandName extends CommandBase {
	...
	public String getUsage(ICommandSender sender) {
		return "§c/"+getName()+" <structureType> [int, max "+maxLoops+"]";
	}
		...
		if (!world.isRemote) {
			...
			for(int i = 0; i < numberToMake; i++) {
				...
				// Not a valid command choice
					...
					sender.sendMessage( new TextComponentString("§c"+getUsage(null) ) );
					break;
				}
				
			}
			sender.sendMessage( new TextComponentString("") );
			sender.sendMessage( new TextComponentString(nameType+":") );
			sender.sendMessage( new TextComponentString(outputString) );
        } 
