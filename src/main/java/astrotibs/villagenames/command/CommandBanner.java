	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
		...
		if (!sender.getEntityWorld().isRemote && sender.getCommandSenderEntity() instanceof EntityPlayer) {
			...
			try {forcedbasemeta = argument.equals("") ? -1 : Integer.parseInt(argument);}
				...
				sender.sendMessage( new TextComponentString("Unknown color: " + argument) );
				return;
			}
			...
			{
				sender.sendMessage( new TextComponentString("Unknown color: " + forcedbasemeta) );
				return;
			}
			...
			{
				Object[] newRandomBanner = BannerGenerator.randomBannerArrays(forcedbasemeta);
