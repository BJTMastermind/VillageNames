...
public class CommandBanner extends CommandBase {
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
		...
		if (!sender.getEntityWorld().isRemote && sender.getCommandSenderEntity() instanceof EntityPlayer) {
			...
			if (argument.toLowerCase().equals("village") && sender.getCommandSenderEntity() != null)
			{
			...
			try {forcedbasemeta = argument.equals("") ? -1 : Integer.parseInt(argument);}
				...
				sender.addChatMessage( new TextComponentString("Unknown color: " + argument) );
				return;
			}
			...
			{
				sender.addChatMessage( new TextComponentString("Unknown color: " + forcedbasemeta) );
				return;
			}
