	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
		...
		if (!sender.getEntityWorld().isRemote && sender.getCommandSenderEntity() instanceof EntityPlayer) {
			...
			if (arguments[0].toLowerCase().equals("village") && sender.getCommandSenderEntity() != null)
			{
				...
				{
					sender.addChatMessage( new TextComponentString("Not inside a village.") );
					return;
				}
			}
			...
			for (int i=0; i<arguments.length; i++)
			{
				...
				{
					sender.addChatMessage( new TextComponentString("Unknown color: -1") );
					return;
				} 
			...
			{
				sender.addChatMessage( new TextComponentString("Unknown color: " + arguments[0]) );
				return;
			}
			try {forcedmeta2 = arguments[1].equals("") ? -1 : Integer.parseInt(arguments[1]);}
			catch (Exception e)
			{
				sender.addChatMessage( new TextComponentString("Unknown color: " + arguments[1]) );
				return;
			}
			...
			{
				sender.addChatMessage( new TextComponentString("Unknown color: " + forcedmeta) );
				return;
			}
			// User submitted a non-color meta number
			if (forcedmeta2 > 15 || forcedmeta2 < -1)
			{
				sender.addChatMessage( new TextComponentString("Unknown color: " + forcedmeta2) );
				return;
			}
