...
public class NetworkHelper
{
    ...
    public static void sendModernVillagerSkinMessage(int villagerID, ExtendedVillager properties, EntityPlayer target) {
        		...
        		) 
        {
            MessageModernVillagerSkin message = new MessageModernVillagerSkin(
            		...
            		properties.getProfessionLevelVN(), // Added in v3.1trades
            		properties.getCareerVN() // Added in v3.1trades
            		);
    ...
    public static void sendZombieVillagerProfessionMessage(int zombieId, ExtendedZombieVillager properties, EntityPlayer target) {
        if (
        		( (zombieId > 0 && properties != null && properties.getProfession() >= 0 && properties.getCareer() > 0) )
