...
//		builder.put(TILE_VILLAGE_CHURCH, 6);
	...
	private void visitVillage(World world, NBTTagCompound tag)
	{
		...
		{
			LogHelper.info(
					"Visiting NPC Village in dimension #"+world.provider.getDimension()+
		...
		for (int i = 0; i < children.tagCount(); i++)
		{
			...
			int x = (boundingBox.minX + boundingBox.maxX + 1)/2;
			int y = (boundingBox.minY + boundingBox.maxY + 1)/2;
			int z = (boundingBox.minZ + boundingBox.maxZ + 1)/2;
			int chunkX = x >> 4;
			int chunkZ = z >> 4;
			...
			if (isTownCenter(childID))
			{
				...
				for (int j = -1; j <= 1; j++)
				{
					for (int k = -1; k <= 1; k++)
					{
						List<Marker> markers = AntiqueAtlasMod.globalMarkersData.getData().getMarkersAtChunk(world.provider.getDimension(), j + chunkX / MarkersData.CHUNK_STEP, k + chunkZ / MarkersData.CHUNK_STEP);
...
//			}
	...
	private static void removeVillage(World world, NBTTagCompound tag)
	{
		...
		for (int i = 0; i < children.tagCount(); i++)
		{
			...
			if (isTownCenter(childID))
			{
				List<Marker> markers = AntiqueAtlasMod.globalMarkersData.getData().getMarkersAtChunk(world.provider.getDimension(), chunkX / MarkersData.CHUNK_STEP, chunkZ / MarkersData.CHUNK_STEP);
