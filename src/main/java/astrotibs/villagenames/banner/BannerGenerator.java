...
public class BannerGenerator
{
    ...
    public static ItemStack makeBanner(@Nullable ArrayList<String> patterns, @Nullable ArrayList<Integer> patternColors)
    {
        ...
        NBTTagCompound nbttagcompound1 = new NBTTagCompound();//= bannerstack.getSubCompound("BlockEntityTag");
        ...
        nbttagcompound1.setInteger("Base", patternColors.get(0) & 15);
    	// Added for v1.11: Banners use top-level "Damage" instead of the "Base" value 
    	if (nbttagcompound1.hasKey("Base"))
    	{
    		bannerstack.setItemDamage(nbttagcompound1.getInteger("Base"));
    		//bannertagcompound.removeTag("Base");
    	}
        ...
        for (int i=0; i < patterns.size(); i++)
        {
            ...
            patterntag.setInteger("Color", patternColors.get(i+1) & 15);
     ...
     */
    public static ItemStack makeBanner(NBTTagCompound bannertagcompound, @Nullable String name)
    {
    	ItemStack bannerstack = new ItemStack(Items.BANNER);
    	
    	// Added for v1.11: Banners use top-level "Damage" instead of the "Base" value 
    	if (bannertagcompound.hasKey("Base"))
    	{
    		bannerstack.setItemDamage(bannertagcompound.getInteger("Base"));
    		//bannertagcompound.removeTag("Base");
    	}
    	...
    	if (!(name==null) && !name.equals("")) {bannerstack.setStackDisplayName(name + " Banner");}
    	
    	return bannerstack;
    }
    
    /**
     * Creates a shield with a banner design and name.
     * Inputs an NBTTagCompound containing the banner pattern and color: specifically, BlockEntityTag
     */
    public static ItemStack makeShieldWithBanner(NBTTagCompound bannertagcompound, @Nullable String name)
    {
    	ItemStack bannerstack = new ItemStack(Items.SHIELD);
    	
    	bannerstack.setTagInfo("BlockEntityTag", bannertagcompound);
    	
    	if (!(name==null) && !name.equals("")) {bannerstack.setStackDisplayName(name + " Shield");}
	...
	public static Object[] getVillageBannerData(EntityLivingBase entity) // v3.1.1 - Changed to EntityLivingBase to allow the player to be an argument
	{
		...
		if (!entity.world.isRemote)
		{
    		...
    		Object[] newRandomBanner = BannerGenerator.randomBannerArrays(entity.world.rand, -1, -1);
			...
			Village villageNearTarget = entity.world.getVillageCollection().getNearestVillage(entity.getPosition(), EntityInteractHandler.villageRadiusBuffer);
			...
			if (villageNearTarget != null)
			{
				...
				VNWorldDataStructure data = VNWorldDataStructure.forWorld(entity.world, "villagenames3_Village", "NamedStructures");
		        ...
		        while(itr.hasNext()) { // Going through the list of VN villages
		            ...
		            Village villageNearSign = entity.world.getVillageCollection().getNearestVillage(new BlockPos(townX, townY, townZ), EntityInteractHandler.villageRadiusBuffer);
		            ...
		            if (villageNearSign == villageNearTarget) { // There is a match between the nearest village to this villager and the nearest village to the sign
			    		...
			    		Random deterministic = new Random(); deterministic.setSeed(entity.world.getSeed() + FunctionsVN.getUniqueLongForXYZ(townX, townY, townZ));
		        ...
		        if (!signLocated) {
		        	...
		        	Random deterministic = new Random(); deterministic.setSeed(entity.world.getSeed() + FunctionsVN.getUniqueLongForXYZ(centerX, centerY, centerZ));
